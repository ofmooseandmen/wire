Feature: Interfacing with the receiver to control applications
    ...
    The following tests launch the default media receiver application (CC1AD845) which is always installed on the device.

  @EmulatedDevice @RealDevice
  Scenario: Application availability
    Given the connection with the device has been opened
    When the availability of application "CC1AD845" is requested
    Then the device reports that the application "CC1AD845" is available
